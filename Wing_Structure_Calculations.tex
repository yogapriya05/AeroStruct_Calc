...
where e = elastic axis to aerodynamic-centre offset. With the main spar at 25 \% chord and aerodynamic centre at 25 \% chord for the WE3.55-9.3 airfoil, the structural elastic axis coincides with the aerodynamic centre, giving a very large divergence speed (see §12.5.1).
...
For the WE3.55-9.3 wing with the spar at the aerodynamic centre (25\%c), the effective divergence eccentricity is small; the divergence speed far exceeds the design dive speed. $\checkmark $
...
\hline
Airfoil & - & WE3.55-9.3 & 9.3 \% t/c, 3.55 \% camber; refer No.14.3 \\
\hline
Maximum thickness & $t_{\max}$ & 2.325 cm = 23.25 mm & 9.3 \% $\times$ chord = $0.093 \times 250$ mm \\
\hline
Chord & $c$ & 0.25 m = 250 mm & Consistent with main analysis (No.2.1) \\
\hline
Wing span (planform length) & $b$ & 1.00 m & Design requirement \\
\hline
Number of ribs & $N$ & 14 & One per 76.9 mm bay; refer No.14.4 \\
\hline
Spar diameter (both spars) & $d$ & 5 mm & Aluminium rod; refer No.14.2 \\
\hline
Wing area & $S$ & $b \times c = 0.25$ m$^{2}$ & Rectangular planform \\
\hline
Aspect ratio & $AR$ & $b^{2} / S = 1.00 / 0.25 = 4.00$ & - \\
\hline
\end{tabularx}
\end{center}
...
\hline
\textbf{Airfoil templates} & WE3.55-9.3 (cut from balsa sheet) & Chord 250 mm $\times$ span-width 2 mm & 14 & ribs & refer No.14.3 \\
\hline
...
Each sheet yields complete airfoil rib assemblies for WE3.55-9.3 at chord = 250 mm. Because the chord (250 mm) exceeds the sheet dimension (200 mm), each rib is cut in two halves: a leading-edge half (0--50\% chord = 125 mm from LE) and a trailing-edge half (50--100\% chord = 125 mm), each fitting within the 200 mm sheet with the grain running along the chord direction. The two halves are joined at the 50\% chord station using a thin 2 mm ${\times}$ 10 mm ${\times}$ 30 mm balsa doubler strip bonded with CA glue. Height-wise, the maximum airfoil height of 23.25 mm is well within the 200 mm sheet. Each sheet provides approximately 7 rib assemblies; two sheets provide 14 ribs --- exactly the design quantity.
...
\item \textbf{Machinability.} Balsa cuts cleanly with a scalpel or laser cutter, allowing precise WE3.55-9.3 profiles to be cut without specialist tooling.
...
\item \textbf{Sheet size (200 ${\times}$ 200 mm) justification.} WE3.55-9.3 at chord = 250 mm has a maximum width (chord direction) of 250 mm --- larger than the 200 mm sheet. Each rib is assembled from two halves joined at mid-chord (50\% chord = 125 mm from LE). This joint falls between the two spar notches (front spar at 25\% = 62.5 mm; rear spar at 65\% = 162.5 mm), preserving full notch integrity. The joint is reinforced with a 2 mm ${\times}$ 10 mm ${\times}$ 30 mm balsa doubler bonded with CA glue. Two sheets provide exactly 14 ribs with minimal offcuts.
\end{enumerate}

\subsection{Airfoil: WE3.55-9.3}

\textbf{Specification:} WE3.55-9.3 cambered airfoil (9.3\% thickness, 3.55\% camber); 14 ribs at chord = 250 mm.
...
Max thickness & t/c & 9.3\% \\
\hline
Max thickness location & - & $\approx$ 30\% chord = 75 mm from LE \\
\hline
Max camber & f/c & 3.55\% \\
\hline
Max camber location & - & $\approx$ 40\% chord = 100 mm from LE \\
\hline
Pitching moment (about AC) & $C_{M,AC}$ & $\approx -0.05$ \\
\hline
Design $C_L$ & - & 0.80 \\
\hline
Aerodynamic centre & $x_{AC}/c$ & 0.25 \\
\hline
\end{tabular}
\end{center}

\subsubsection{Key WE3.55-9.3 cross-section parameters at c = 250 mm}
...
\subsubsection{Why WE3.55-9.3?}

\begin{enumerate}[noitemsep,topsep=2pt]
\item \textbf{Performance.} The WE3.55-9.3 was used in the original experiments and calculations for this solar UAV design, providing validated aerodynamic data at the design Reynolds number.
\item \textbf{Camber benefit.} With $f/c = 3.55\%$ camber, the airfoil generates $C_L \approx 0.80$ at low angle of attack, reducing induced drag at cruise.
\item \textbf{Low Reynolds number.} Designed to operate efficiently at Re $\approx 10^5$, which matches the cruise condition of our 1 m span prototype.
\item \textbf{Structural depth.} The 9.3\% thickness provides $t_{\max} = 23.25$ mm at the front spar, comfortably accommodating a $\varnothing$ 5 mm spar rod.
\item \textbf{Template availability.} WE3.55-9.3 ordinates are available from standard airfoil databases (UIUC Airfoil Coordinate Database) for laser-cut or hand-cut rib templates.
\end{enumerate}
...
\hline
Chord $c$ & 0.25 m & Consistent with main analysis (No.2.1); WE3.55-9.3 at $t_{\max} = 23.25$ mm: $c = 23.25/0.093 = 250$ mm \\
\hline
