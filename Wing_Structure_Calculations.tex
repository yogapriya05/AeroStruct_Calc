...
\large Solar UAV Wing --- AE322\,/\,AE341 Group~2, IIST\\
...
\item Introduction and Use Case
\item Wing Geometry and Airfoil Parameters
\item Mass Estimation and Assumptions
\item Aerodynamic Load Analysis
\end{enumerate}
\begin{itemize}[noitemsep,topsep=2pt]
\item 4.3 Design Loads (with Gust Alleviation Factor K\_g)
\item 4.4 Flight Envelope (V-n Diagram)
\item 4.5 Aileron Roll Manoeuvre Load
\item 4.6 Aerodynamic Loads at ISA 1000 m
\end{itemize}
\begin{enumerate}[noitemsep,topsep=2pt]
\item Distributed Lift and Shear Force Diagrams
\end{enumerate}
\begin{itemize}[noitemsep,topsep=2pt]
\item 5.4 Inertia Relief
\end{itemize}
\begin{enumerate}[noitemsep,topsep=2pt]
\item Bending Moment Distribution
\item Torsion Analysis
\item Main Spar (Front Spar) Sizing
\item Rear Spar Sizing
\item Rib Design and Spacing
\item Skin (Covering Sheet) Sizing
\item Natural Frequency and Vibration Analysis
\begin{itemize}[noitemsep]
\item 12.5 Flutter and Divergence Speed
\end{itemize}
\item Summary of All Substructures
\item Fabrication --- Bill of Materials
\item Testing: Procedures, Cautions, and Expected Results
\end{enumerate}
\begin{itemize}[noitemsep,topsep=2pt]
\item Appendix A --- Material Properties
\item Appendix B --- Assumptions Correction Table
\item Appendix C --- Full-Scale Reference Analysis (Sky-Sailor)
\end{itemize}
...
\section{Wing Geometry and Airfoil Parameters}



\subsection{2.1 Wing planform}
...
\subsection{2.2 Airfoil: WE3.55-9.3}
...
\subsection{2.3 Spar and rib layout}
...
\section{Mass Estimation and Assumptions}
...
\hline
\textbf{A1.} Total aircraft mass m = 0.50 kg & Scaled from Sky-Sailor using cubic scaling law (see \S{}3.2). Conservative estimate for a 980 mm solar UAV. \\
\hline
\textbf{A2.} Design load factor n = 2.5 & Standard value for small UAVs (CS-23 small aircraft; acceptable for academic model). Gust correction applied in \S{}4.3. \\
\hline
\textbf{A3.} Ultimate factor of safety FOS = 1.5 & FAR/CS standard; ensures plastic reserve beyond limit loads. \\
\hline
\textbf{A4.} Lift acts at x\_CP = 31.3 \% c & Derived from C\_L and C\_M (see \S{}4.2); corrected for off-design conditions. \\
\hline
\textbf{A5.} Rectangular wing ${\rightarrow}$ uniform load baseline & Elliptical correction applied in \S{}5.2. \\
\hline
...
\subsection{3.2 Total aircraft mass --- scaling from Sky-Sailor}
...
\subsection{4.1 Cruise flight conditions}
...
\subsection{4.2 Centre of pressure location}
...
\subsection{4.3 Design loads (limit and ultimate)}
...
\textbf{Gust alleviation factor (FAR 23 \S{}23.341):} The raw gust increment overestimates the effective load because inertia of the aircraft partially attenuates the instantaneous gust response. The alleviation factor K\_g is:
...
\begin{quote}\itshape
\textbf{Note:} Even the alleviated gust load factor (3.43) exceeds the 2.5 manoeuvre limit. This is typical for ultra-light solar UAVs with very low wing loading (W/S = 20 N/m$^{2}$). Operationally, flights must be restricted to calm conditions (winds ${\leq}$ 5 m/s) to keep the effective load factor within the structural envelope.
\end{quote}
...
\subsection{4.4 Flight Envelope (V-n Diagram)}
...
\textbf{Design dive speed} (FAR 23 \S{}23.335):
...
\subsection{4.5 Aileron Roll Manoeuvre Load}
...
From Section 8.2, the rear spar root bending moment at symmetric limit is M\_RS,limit = 0.191 N${\cdot}$m (10 \% of total, from \S{}9.1). With aileron increment:
...
\subsection{4.6 Aerodynamic Loads at Operating Altitude (ISA 1000 m)}
...
\subsubsection{4.6.1 ISA atmospheric properties at 1000 m}
...
\subsubsection{4.6.2 Cruise speed at 1000 m}
...
\subsubsection{4.6.3 Dynamic pressure and Reynolds number at 1000 m}
...
\begin{quote}\itshape
\textbf{Note:} Dynamic pressure at 1000 m (25.03 N/m$^{2}$) is virtually identical to the sea-level value (25.01 N/m$^{2}$). This is expected --- for level flight the required lift equals weight, so $q_\infty\,C_L\,S = W$ regardless of altitude, meaning $q_\infty$ is fixed by the aircraft weight and wing parameters, not by altitude directly.
\end{quote}
...
The Reynolds number decreases slightly relative to sea level (1.09 ${\times}$ 10$^{5}$), remaining within the valid operating envelope of the WE3.55-9.3 airfoil (80,000 -- 300,000). Transition behaviour may shift marginally, but no change in structural sizing is required.


\subsubsection{4.6.4 Aerodynamic loads at 1000 m --- summary}
...
\section{Distributed Lift and Shear Force Diagrams}


The wing coordinate y runs from root (y = 0) to tip (y = b/2 = 0.490 m).


\subsection{5.1 Uniform load (rectangular wing, conservative)}
...
\subsection{5.2 Elliptical load distribution (Prandtl lifting-line correction)}
...
\subsection{5.3 Shear force diagram V(y) --- ultimate loads}
...
The inertia relief reduces the root bending moment by \textbf{~30 \%} relative to the aerodynamic-only value. For structural sizing the \textbf{conservative unreduced value (M\_root,ult = 2.025 N${\cdot}$m)} is used (consistent with \S{}6 and \S{}7 design calculations), making inertia relief an additional safety margin.
...
\section{Bending Moment Distribution}



\subsection{6.1 Bending moment M(y) --- ultimate loads}
...
\subsection{6.2 Section modulus requirement}
...
\section{Main Spar (Front Spar) Sizing}


The main spar is a \textbf{solid circular aluminium rod} (Al 6061-T6).


\subsection{7.1 Material properties --- Al 6061-T6}
...
\subsection{7.2 Minimum spar radius from bending}
...
\subsection{7.3 Shear check on main spar}
...
\subsection{7.4 Spar deflection at tip}
...
\section{Rib Design and Spacing}



\subsection{9.1 Number of ribs and spacing}
...
\subsection{9.3 Rib as a simply-supported beam}
...
\subsection{9.4 Rib cross-section sizing (balsa wood)}
...
\subsection{9.5 Skin-buckling check (rib spacing)}
...
\item $\nu$ = 0.33 (aluminium)
\item $t_s$ = skin thickness (to be determined in \S{}11)
\item $a$ = rib spacing = 25.1 mm
\end{itemize}

Setting $\sigma_{\text{cr}} = \sigma_{\text{allow}}$ and solving for $t_s$ will be done in \S{}11.


\subsection{9.6 Rib mass}
...
\section{Torsion Analysis}



\subsection{10.1 Applied aerodynamic torque}
...
\item $e_L$ = 15.6 mm = 0.0156 m (lift eccentricity from FS, \S{}4.2)
...
\[
= 0.1435 - 0.0383 = 0.1052\ \text{N}\cdot\text{m}
\]
...
\subsection{10.2 D-box (closed section) torsional stiffness}
...
For aluminium skin with $t_s = 0.3\ \text{mm}$ (see \S{}11):
...
\subsection{10.3 Angle of twist}
...
\[
= \frac{4 \times 2.410 \times 10^{-7} \times 7.8 \times 10^6}{72.9 \times 10^{-3}} = \frac{7.519}{72.9 \times 10^{-3}} = 103.1\ \text{N}\cdot\text{m}^2
\]
...
\section{Skin (Covering Sheet) Sizing}



\subsection{11.1 Leading-edge D-box skin}


The D-box skin must resist:
\begin{enumerate}[noitemsep,topsep=2pt]
\item Torsion-induced shear (\S{}10.2): ${\tau}$ = 0.357 MPa
...
The practical design is: \textbf{0.3 mm aluminium sheet} for the D-box and leading-edge, with \textbf{thin polyester film} (Oracover / Monokote) for the inter-spar and trailing-edge bay, bonded to the ribs. This is consistent with Sky-Sailor construction.
...
\section{Natural Frequency and Vibration Analysis}



\subsection{12.1 Fundamental bending frequency}
...
\subsection{12.2 Second bending mode}
...
\subsection{12.3 Fundamental torsional frequency}
...
\subsection{12.4 Aileron buzz / control surface frequency}
...
\subsection{12.5 Flutter and Divergence Speed (Simplified)}
...
\subsubsection{12.5.1 Torsional Divergence Speed}
...
\subsubsection{12.5.2 Bending-Torsion Flutter Speed (Frequency-Ratio Method)}
...
\subsection{13.1 Substructure count and function}
...
\hline
\textbf{Front (main) spar} & 2 (one per half-wing) & Al 6061-T6 rod & $\varnothing$ 5 mm, L = 490 mm & 52 & Bending \& shear; 90\% of M\_root \\
\hline
...
\hline
\textbf{Leading-edge D-box skin} & 1 continuous & Al sheet 0.3 mm & LE to FS (62.5 mm), span 980 mm & ~50 & Primary torsion box; aero shape \\
\hline
\textbf{Inter-spar upper skin} & 1 continuous & Polyester film or Al 0.1 mm & FS to RS (100 mm), span 980 mm & ~10 & Shear diaphragm; solar cell substrate \\
\hline
\textbf{Inter-spar lower skin} & 1 continuous & Polyester film & FS to RS, span 980 mm & ~5 & Shear diaphragm; aerodynamic surface \\
\hline
\textbf{Trailing-edge skin} & 1 continuous & Polyester film & RS to TE (87.5 mm), span 980 mm & ~5 & Aerodynamic shape \\
\hline
\textbf{Root joint / splices} & 2 (root of each half) & Al bracket & --- & ~10 & Load transfer to fuselage \\
\hline
\textbf{Ailerons} & 2 (one per half-wing) & Balsa + film & ~75 mm chord, ~300 mm span & ~15 & Roll control \\
\hline
\textbf{Wing tip fairings} & 2 & Moulded Depron / foam & --- & ~5 & Tip vortex reduction \\
\hline
\textbf{Total (structural wing)} & --- & --- & --- & \textbf{~203 g} & --- \\
\hline
\end{tabular}
\end{center}


\subsection{13.2 Structural margin summary}
...
\subsection{13.3 Natural frequencies}
...
\section{Fabrication --- Bill of Materials}
...
\subsection{14.1 Updated Prototype Geometry}
...
\hline
Airfoil & --- & NACA 0012 & 12 \% symmetric; see \S{}14.3 \\
\hline
Maximum thickness & t\_max & 3.6 cm = 36 mm & 12 \% ${\times}$ chord (see \S{}14.3) \\
\hline
Chord & c & 0.30 m = 300 mm & t\_max / 0.12 = 36/0.12 = 300 mm \\
\hline
Wing span (planform length) & b & 1.00 m & Design requirement \\
\hline
Number of ribs & N & 12 & One per 91 mm bay; see \S{}14.4 \\
\hline
Spar diameter (both spars) & d & 5 mm & Aluminium rod; see \S{}14.2 \\
\hline
...
\subsection{14.2 Bill of Materials}
...
\hline
\textbf{Front (main) spar} & Aluminium alloy rod (Al 6061-T6) & $\varnothing$ 5 mm ${\times}$ 1000 mm & 2 & pieces & See \S{}14.2.1 \\
\hline
\textbf{Rear spar} & Aluminium alloy rod (Al 6061-T6) & $\varnothing$ 5 mm ${\times}$ 1000 mm & 2 & pieces & See \S{}14.2.1 \\
\hline
\textbf{Rib blanks} & Balsa wood sheet (medium density, ~160 kg/m$^{3}$) & 200 mm ${\times}$ 200 mm ${\times}$ 2 mm & 2 & sheets & See \S{}14.2.2 \\
\hline
\textbf{Airfoil templates} & NACA 0012 (cut from balsa sheet) & Chord 300 mm ${\times}$ span-width 2 mm & 12 & ribs & See \S{}14.3 \\
\hline
...
\subsubsection{14.2.1 Spars --- Aluminium Rod, Diameter 5 mm}
...
\textbf{Structural adequacy --- front spar:} From Section 7.2, the minimum front spar diameter for the 980 mm / 250 mm chord analysis wing is d\_min = 4.64 mm (r\_min = 2.32 mm). For the updated 1 m span / 300 mm chord prototype, using n = 2.5, FOS = 1.5 at sea level / 1000 m (loads unchanged per \S{}4.4), the uniform-load root bending moment is:

\[
M_{\text{root,ult}} = \frac{w_0\,(b/2)^2}{2} = \frac{(9.20/0.500)\times(0.500)^2}{2} = \frac{18.40 \times 0.250}{2} = 2.30\ \text{N}\cdot\text{m}
\]

Minimum front-spar radius (${\sigma}$\_allow = ${\sigma}$\_ult / FOS = 310 / 1.5 = 206.7 MPa from \S{}7.1; 90 \% of moment to front spar):
...
Margin: $MS_{RS} = 206.7/18.7 - 1 = +10.1$ $\checkmark $ --- The rear spar is structurally over-designed at $\varnothing$ 5 mm; it is specified at this diameter purely for \textbf{procurement standardisation} (one rod diameter for both spars).
...
\subsubsection{14.2.2 Balsa Wood Sheets --- 200 ${\times}$ 200 mm, Quantity 2}
...
\subsection{14.3 Airfoil: NACA 0012}
...
\subsection{14.4 Wing Planform --- 1 m Span}
...
\hline
Chord c & 0.30 m & Set by NACA 0012 at t\_max = 3.6 cm: c = 36 mm / 0.12 = 300 mm \\
\hline
Aspect ratio AR & b$^{2}$/S = 1.00 / (1.00 ${\times}$ 0.30) = \textbf{3.33} & Moderate AR typical of prototype demonstration wings \\
\hline
...
\textbf{Why 1 m span?} A 1 m span is the standard for an academic laboratory prototype for several reasons:
\begin{itemize}[noitemsep,topsep=2pt]
...
\section{Testing: Procedures, Cautions, and Expected Results}
...
\subsection{15.2 Phase A --- Material and Coupon Tests}
...
\textbf{Purpose:} Verify the tensile and bending strength of the purchased Al 6061-T6 rod ($\varnothing$ 5 mm) matches the assumed material properties (${\sigma}$\_y = 276 MPa, ${\sigma}$\_ult = 310 MPa, E = 69 GPa).
...
\hline
Yield load (100 mm free length) & F\_y ${\approx}$ 33.9 N & 30.0 -- 37.8 N \\
\hline
...
\textbf{Purpose:} Confirm the along-grain bending modulus and modulus of rupture (MOR) of the balsa sheet batch.
...
\subsection{15.3 Phase B --- Component Tests}
...
\textbf{Purpose:} Verify that the assembled balsa ribs maintain their profile under aerodynamic pressure loads.
...
\textbf{Purpose:} Verify spar stiffness and confirm linear-elastic response over the 500 mm half-span.
...
\subsection{15.4 Phase C --- Full Wing Static Test}
...
\begin{quote}\itshape
\textbf{Note before setup:} The $\varnothing$ 5 mm front spar remains fully elastic at all design load levels. The root front-spar bending moment at design ultimate load (2.07 N${\cdot}$m) is only 61 \% of the yield moment (3.39 N${\cdot}$m), providing a yield safety margin of +64 \%. The full wing may therefore be safely loaded to 100 \% design limit load (6.13 N half-wing) and, if desired, to design ultimate load (9.20 N half-wing) during structural testing. Standard load increments (${\leq}$ 10 \% of ultimate per 30-second step) and general safety precautions from \S{}15.1 still apply.
\end{quote}

\textbf{Purpose:} Verify linear-elastic structural response of the assembled wing and measure tip deflection up to design limit loads; confirm structural integrity and no damage (debonds, cracks) under the full design load range.
...
\textbf{Purpose:} Verify the D-box torsional stiffness and confirm that the wing does not twist excessively under aerodynamic pitching moment.
...
\item Target torque: 0.05 N${\cdot}$m (half the root torque calculated in \S{}10.1).
...
Analytical twist (from \S{}10.3): $\phi = 0.014^{\circ}$ for T\_root = 0.105 N${\cdot}$m. Values here are lower because load is applied at 75 \% span, not root.
...
\textbf{Purpose:} Detect any adhesive delamination or debonds between skin, ribs, and spars without destructive testing.
...
\subsection{15.5 Phase D --- Vibration / Modal Test}
...
\textbf{Purpose:} Confirm the first bending natural frequency is consistent with the analytical prediction (f$_{1}$ ${\approx}$ 5.97 Hz from \S{}12.1).
...
\subsection{15.6 Test Record and Sign-Off}
...
\begin{tabular}{|l|l|l|l|l|l|}
\hline
Material & E (GPa) & G (GPa) & ${\sigma}$\_y (MPa) & ${\sigma}$\_ult (MPa) & ${\rho}$ (kg/m$^{3}$) \\
\hline
...
\section{Appendix B --- Assumptions Correction Table}
...
\hline
C\_P location & 25\% chord & \textbf{31.3\% chord} & From C\_M,AC = -0.05, C\_L = 0.80 \\
\hline
D-box area & Not estimated & \textbf{490.9 mm$^{2}$} & Semi-ellipse geometry of airfoil LE \\
\hline
Gust load factor & n = 2.5 & ${\Delta}$n\_gust = 7.31, \textbf{limited to n = 2.5 operationally} & Calm-weather flight restriction \\
\hline
Skin in bending & Participates & \textbf{Spar only} (skin is secondary) & Comparison of skin/spar stiffness \\
\hline
Torsional stiffness & Open section & \textbf{Closed D-box} (Bredt--Batho) & LE + FS + skin form closed cell \\
\hline
\end{tabular}
\end{center}
...
\section{Appendix C --- Full-Scale Reference Analysis (Sky-Sailor)}
...
\subsection{C.1 Full-Scale Wing Parameters}
...
Using FAR 23 \S{}23.337 / \S{}23.341 with K\_g:
...
\subsection{C.2 Full-Scale Structural Loads}


\textbf{Inertia-relieved net load per semi-span (n = 3.5):}

\begin{verbatim}
L_per_wing = (n x W) / 2 = (3.5 x 23.96) / 2 = 41.93 N
m_wing_semi ~ 0.2625 kg  ->  W_inertia = 0.2625 x 9.81 x 3.5 = 9.01 N
...
\subsection{C.3 Full-Scale Main Spar (CFRP I-Beam)}
...
Spar cross-section:
  ...
  Web height h_web = h - 2 x t_cap = 21.25 mm
\end{verbatim}
...
Step-by-step:
\begin{itemize}[noitemsep,topsep=2pt]
\item Coefficient: $5.35 \times 9.870 \times 3.5 \times 10^9 / (12 \times 0.91) = 1.693 \times 10^{10}$
\item $(t/h)^2 = (2.0/21.25)^2 = (0.09412)^2 = 8.859 \times 10^{-3}$
\item $\tau_{\text{cr}} = 1.693 \times 10^{10} \times 8.859 \times 10^{-3} = 1.500 \times 10^{8}\ \text{Pa}$
\end{itemize}
...
\begin{quote}\itshape
\textbf{Note:} An earlier version of the spar used t\_web = 1.5 mm, which gave ${\tau}$\_cr = 84.4 MPa and SF\_buckling = 174. Even the thinner web is adequate. The web thickness of 2.0 mm was chosen for \textbf{ease of fabrication} (easier to cut and handle) rather than structural necessity.
\end{quote}

\textbf{Full-scale spar structural weight:}

\begin{verbatim}
CFRP caps:  m = rho_CFRP x 2 x b_cap x t_cap x s = 1600 x 2 x 0.015 x 0.001 x 1.6 = 76.8 g
Balsa web:  m = rho_balsa x t_web x h_web x s     = 160 x 0.002 x 0.02125 x 1.6   = 10.9 g
...
\subsection{C.4 Full-Scale Rear Spar (CFRP Tube)}


\begin{verbatim}
Location: 65 % chord = 162.5 mm from LE
Airfoil thickness at 65% chord ~ 6%: t_rear = 0.06 x 250 = 15.0 mm
Section: CFRP rectangular tube 15 x 6 mm, t_wall = 0.5 mm
I_rear = (6x15^3 - 5x14^3) / 12 (mm^4) x 10^(-)^1^2 m^4/mm^4
       = (6x3375 - 5x2744)/12 x 10^(-)^1^2 = (20,250 - 13,720)/12 x 10^(-)^1^2 = 544.2 x 10^(-)^1^2
       ~ 5.44 x 10^(-)^1^0 m^4
\end{verbatim}
...
\subsection{C.5 Full-Scale Natural Frequencies}
...
Mode & Formula & Value \\
\hline
1st bending f$_{1}$ & $\frac{(1.875)^2}{2\pi L^2}\sqrt{EI/\bar{m}}$ with EI = 120${\times}$10$^{9}$${\times}$5.66${\times}$10$^{-}$$^{9}$ = 679.2 N${\cdot}$m$^{2}$, $\bar{m}$ = 0.1641 kg/m, L = 1.6 m & \textbf{7.6 Hz} \\
\hline
1st torsion f\_T & $\frac{\pi}{4L}\sqrt{GJ/I_\alpha}$ with GJ = 12.22 N${\cdot}$m$^{2}$, $I_\alpha$ = 8.62${\times}$10$^{-}$$^{4}$ kg${\cdot}$m & \textbf{18.6 Hz} \\
\hline
f\_T / f\_B & 18.6 / 7.6 = 2.45 & Flutter coupling not expected $\checkmark $ \\
\hline
\end{tabular}
\end{center}


\subsection{C.6 Full-Scale Aeroelastic Margins}
...
\hline
Divergence speed V\_div & $\gg$ V\_D (elastic axis ${\approx}$ AC) & 1.15 ${\times}$ V\_D = 13.4 m/s & $\checkmark $ Infinite margin \\
\hline
Flutter speed V\_flutter & ~47 m/s (simplified estimate) & 1.20 ${\times}$ V\_D = 13.9 m/s & $\checkmark $ ${\times}$3.4 \\
\hline
...
\subsection{C.7 Comparison: Full-Scale vs Scale Prototype}
...
\begin{enumerate}[noitemsep,topsep=2pt]
\item Noth, A. and Siegwart, R., \textit{Design of Solar Powered Airplanes for Continuous Flight}, ETH Zürich, Dissertation No. 18010, 2008.
\item Wing Structure G2 Preliminary Design Report, AE322/AE341, IIST, February 2026.
\item Structures Project VI --- IDR, IIST, 2026.
\item Young, W.C. and Budynas, R.G., \textit{Roark's Formulas for Stress and Strain}, 7th ed., McGraw-Hill, 2002. (Tables 3.1, 8.1, 15.3, 16.1)
\item Megson, T.H.G., \textit{Aircraft Structures for Engineering Students}, 4th ed., Butterworth-Heinemann.
\item Maughmer, M.D. and Brandt, S.A., \textit{Aerodynamics for Engineers}, Prentice Hall.
\item Blevins, R.D., \textit{Formulas for Natural Frequency and Mode Shape}, Van Nostrand Reinhold, 1979.
\item FAR Part 23, \S{}23.335, \S{}23.337, \S{}23.341 --- \textit{Structural Design Criteria for Small Aircraft}, US FAA.
\item Timoshenko, S. and Gere, J., \textit{Theory of Elastic Stability}, 2nd ed., McGraw-Hill, 1961. (Plates and shells buckling coefficients.)
\item Prandtl, L., "Tragflügeltheorie" (\textit{Lifting-Line Theory}), Nachrichten der Gesellschaft der Wissenschaften zu Göttingen, 1918.
\item ICAO Doc 7488 --- \textit{Manual of the ICAO Standard Atmosphere}, 3rd ed., 1993. (ISA tropospheric temperature lapse rate ${\Lambda}$ = 6.5 K/km.)
\end{enumerate}
